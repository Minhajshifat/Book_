...
def profile_view(request):
    purchased_books = PurchaseModel.objects.filter(
        user=request.user.accounts.user, isBorrowed=True
    )
    context = {
...
class BuyBookView(View):
    def get(self, request, book_id):
        ...
        )  # Assuming this gets the related UserAccountModel
        ...
        if user_account.balance >= book.price and book.quantity > 0:
            ...
            purchase = PurchaseModel.objects.create(
                user=user, Book=book
            )  # purchase model a Book variable ache tai ekhaen book likhlam
            messages.success(request, f"You have successfully purchased a book.")
            send_mail_to_user(request.user, "Book Purchase Success", "borrow.html")

            # comment_page_url = reverse('comment_page', kwargs={'id': book.id})
            # return redirect(comment_page_url)
            return render(
                request, "comment_page.html", {"book": book, "comments": comments}
            )
        else:
            messages.warning(
